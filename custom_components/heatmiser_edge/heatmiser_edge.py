...
class heatmiser_edge_register_store:
    def __init__(self, host, modbus_id, unit: "ModbusUnit") -> None:
        _LOGGER.debug("Initialising Register store")
        self.registers = [None] * 218
        self.device_type = None
        self.time_of_next_update = None
        self._slave_id = modbus_id # Kept for logging/identification only
        self._host = host # Kept for logging/identification only
        self._unit = unit
    ...
    async def write_register(self, register: int, value: int, refresh_values_after_writing: bool) -> None:
        ...
        try:
            await self._unit.write_register(int(register), int(value))
    ...
    async def write_register_range(self, start_register: int, values: List[int], refresh_values_after_writing: bool) -> None:
        ...
        try:
            for chunk_start_register_idx in range(0, len(values), MAX_REGISTER_WRITE_COUNT):
                ...
                await self._unit.write_registers(int(start_register) + chunk_start_register_idx, chunk)
        except ModbusError as ex:
    ...
    async def async_update(self) -> None:
        ...
        for i in range(0,210,MAX_REGISTER_UPDATE_COUNT):
            result = await self._unit.read_holding_registers(i, MAX_REGISTER_UPDATE_COUNT)     # get information from device
            register_updated_values[i:i+MAX_REGISTER_UPDATE_COUNT] = result

        result = await self._unit.read_holding_registers(210, 8)     # Do last 8 seperately
        register_updated_values[210:218] = result
    ...
    async def async_update_device_time(self) -> None:
        ...
        if (self.time_of_next_update is None) or (current_time > (self.time_of_next_update)):
            ...
            if int(is_dst) != int(self.registers[int(RegisterAddresses[self.device_type].DAYLIGHT_SAVING_STATUS_RD)]):
                ...
                await self._unit.write_register(int(RegisterAddresses[self.device_type].DAYLIGHT_SAVING_STATUS), int(is_dst))
            await self._unit.write_register(int(RegisterAddresses[self.device_type].SYNCHRONOUS_RTC_YEAR), year)
            await self._unit.write_register(int(RegisterAddresses[self.device_type].SYNCHRONOUS_RTC_MONTH_DAY), month_day)
            await self._unit.write_register(int(RegisterAddresses[self.device_type].SYNCHRONOUS_RTC_HOUR_MINUTE), hour_minute)
            await self._unit.write_register(int(RegisterAddresses[self.device_type].SYNCHRONOUS_RTC_SECOND), second)
